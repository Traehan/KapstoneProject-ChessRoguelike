...
public class DeckViewController : MonoBehaviour
{
    ...
    [Tooltip("Enable ONLY on the persistent-scene copy (PersistentUI) so it always reclaims Instance no matter " +
             "what order scenes happen to load in this play session (e.g. pressing Play directly inside SampleScene). " +
             "Leave off on any other coexisting copy (e.g. SampleScene's battle draw/discard pile viewer).")]
    ...
    void Awake()
    {
        // Multiple DeckViewControllers can legitimately coexist at once (e.g. the persistent
        // RunDeck viewer in PersistentUI, plus SampleScene's own controller for the battle
        // draw/discard piles). Instance is only a convenience fallback for buttons/callers that
        // don't wire a direct reference - see DeckViewOpenButton.controller - so nobody gets
        // destroyed. isPrimaryInstance always wins the claim regardless of load order, so testing
        // by pressing Play inside a non-persistent scene can't leave the wrong copy as Instance.
