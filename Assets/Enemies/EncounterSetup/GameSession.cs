...
public class GameSession : MonoBehaviour
{
    ...
    [Header("Starting Card Draft")]
    [Tooltip("Same odds asset used for victory rewards (e.g. CardRarityOdds_Ascension1thru4) - only its Common/Uncommon weights apply here, Rare is excluded from the starting draft regardless of this asset's rareWeight.")]
    public CardRarityOdds startingDraftOdds;
    [Tooltip("How many distinct random cards are drafted at run start.")]
    public int startingDraftPickCount = 2;
    [Tooltip("How many copies of each drafted card are added to the deck.")]
    public int startingDraftCopiesPerPick = 2;
    public List<CardDefinitionSO> lastGrantedStartingCards = new();

    public bool hasGrantedStartingTroop = false;
    public bool hasGrantedStartingCards = false;
    ...
    public bool hasShownStartingRelicSelection = false;
    public event System.Action<RelicSO> OnRelicAdded;

    [Header("Gauntlet (Elite/Trial) State")]
    [Tooltip("Set by MapGenerator.NavigateToNodeScene right before loading into a flagged Encounter node's battle scene.")]
    public bool pendingGauntletAvailable;
    public GauntletRewardType pendingGauntletReward;
    public GauntletChallengeType pendingGauntletChallenge;
    [Tooltip("Set by GauntletScreen from its accept/decline toggle once the player confirms Start Encounter.")]
    public bool gauntletAccepted;

    [Header("Gauntlet Reward Tuning")]
    public int gauntletGoldReward = 150;
    [Tooltip("Weight for Uncommon vs Rare when rolling the Gauntlet relic reward (skewed above normal odds to make the risk worth it).")]
    public int gauntletRelicUncommonWeight = 65;
    public int gauntletRelicRareWeight = 35;
    ...
    void Awake()
    {
        ...
        GameEvents.OnEncounterWon += HandleEncounterWonForGauntlet;
        GameEvents.OnEncounterLost += HandleEncounterLostForGauntlet;
    }
    ...
    // --- Gauntlet (Elite/Trial) reward granting -------------------------------------------------
    // Subscribed to the same GameEvents.OnEncounterWon TurnManager.VictoryLives.cs already fires.
    // Gated on gauntletAccepted so declining a Gauntlet plays a normal fight with no reward.

    void HandleEncounterWonForGauntlet()
    {
        if (gauntletAccepted)
            GrantGauntletReward(pendingGauntletReward);

        ClearGauntletState();
    }

    void HandleEncounterLostForGauntlet()
    {
        // No reward on a loss either way - just make sure state doesn't leak into the next fight.
        ClearGauntletState();
    }

    void ClearGauntletState()
    {
        pendingGauntletAvailable = false;
        gauntletAccepted = false;
    }

    void GrantGauntletReward(GauntletRewardType reward)
    {
        switch (reward)
        {
            case GauntletRewardType.Gold:
                if (CurrencyManager.Instance != null)
                    CurrencyManager.Instance.AddCoins(gauntletGoldReward);
                else
                    Debug.LogWarning("[GameSession] Gauntlet Gold reward could not be granted: CurrencyManager.Instance is null.");
                break;

            case GauntletRewardType.QueenMove:
                AddMapMovementCount(MapMovementType.Queen, 1);
                break;

            case GauntletRewardType.Relic:
                GrantGauntletRelicReward();
                break;
        }

        Debug.Log($"[GameSession] Gauntlet reward granted: {reward}.");
    }

    void GrantGauntletRelicReward()
    {
        if (relicDatabase == null)
        {
            Debug.LogWarning("[GameSession] Gauntlet Relic reward could not be granted: relicDatabase is null.");
            return;
        }

        RelicRarity rarity = RollGauntletRelicRarity();
        var drawn = relicDatabase.DrawRandom(rarity, selectedClan, 1, ownedRelics);

        // Fall back to Common if the skewed pool is exhausted (e.g. all Uncommon/Rare relics already owned).
        if ((drawn == null || drawn.Count == 0) && rarity != RelicRarity.Common)
            drawn = relicDatabase.DrawRandom(RelicRarity.Common, selectedClan, 1, ownedRelics);

        if (drawn != null && drawn.Count > 0)
            AddRelic(drawn[0]);
        else
            Debug.LogWarning("[GameSession] Gauntlet Relic reward: no eligible relics left to draw.");
    }

    RelicRarity RollGauntletRelicRarity()
    {
        int total = Mathf.Max(0, gauntletRelicUncommonWeight) + Mathf.Max(0, gauntletRelicRareWeight);
        if (total <= 0) return RelicRarity.Uncommon;

        int roll = Random.Range(0, total);
        return roll < gauntletRelicUncommonWeight ? RelicRarity.Uncommon : RelicRarity.Rare;
    }
    ...
    public void StartNewRun(ClanDefinition clan)
    {
        ...
        hasGrantedStartingCards = false;
        lastGrantedStartingCards.Clear();
        hasShownStartingTroopPopup = false;
        hasShownStartingRelicSelection = false;
    ...
    // Monster Train-style start-of-run draft: rolls startingDraftPickCount distinct cards from the
    // clan's Common/Uncommon pool (Rare and above are excluded - those are for victory rewards, not a
    // guaranteed run-start grant) and adds startingDraftCopiesPerPick copies of each to CurrentRunDeck.
    // Returns the distinct picks (not the copies) so the run-start popup can show "xN" per card.
    public List<CardDefinitionSO> GrantRandomStartingCards()
    {
        if (hasGrantedStartingCards)
            return lastGrantedStartingCards;

        hasGrantedStartingCards = true;
        lastGrantedStartingCards.Clear();

        if (selectedClan == null || selectedClan.SpellPool == null || selectedClan.SpellPool.Length == 0)
            return lastGrantedStartingCards;

        var picks = CardDraftUtility.RollDistinct(
            selectedClan.SpellPool,
            startingDraftPickCount,
            startingDraftOdds,
            r => r == CardRarity.Common || r == CardRarity.Uncommon);

        foreach (var card in picks)
        {
            for (int i = 0; i < startingDraftCopiesPerPick; i++)
                CurrentRunDeck.Add(card);

            lastGrantedStartingCards.Add(card);
        }
    ...
    PieceDefinition extraArmyPiece = null)
{
    ...
    public bool HasRelic(RelicSO relic) => relic != null && ownedRelics.Contains(relic);
    ...
    public bool AddRelic(RelicSO relic)
    {
        ...
        Debug.Log($"[GameSession] Relic added: {relic.displayName} ({relic.rarity}). Owned relics: {ownedRelics.Count}.");
        return true;
    }
}