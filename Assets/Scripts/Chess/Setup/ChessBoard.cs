...
namespace Chess
{
    public class ChessBoard : MonoBehaviour
    {
        ...
        public bool IsOccupied(Vector2Int c)
        {
            ...
            {
                if (p == null || p.Coord != c) { _pieces.Remove(c); return false; }
                return true;
            }
            return false;
        }
        ...
        public Piece GetPiece(Vector2Int c)
        {
            if (_pieces.TryGetValue(c, out var p))
            {
                if (p == null || p.Coord != c) { _pieces.Remove(c); return null; }
                return p;
            }
            return null;
        }
        ...
        public void Rebuild()
        {
            ...
            // clear occupancy map to remove “ghost” pieces
            _pieces.Clear();
        ...
        public bool TryMoveWithCapture(Piece mover, Vector2Int to, out Piece captured)
        {
            ...
            if (IsOccupied(to))
            {
                ...
                if (there == null) _pieces.Remove(to);
                else if (there.Team == mover.Team) return false;
                else { captured = there; CapturePiece(captured); }
            }
        ...
        public void PlaceWithoutCapture(Piece p, Vector2Int to)
        {
            ...
            if (p == null) { _pieces.Remove(to); return; }
